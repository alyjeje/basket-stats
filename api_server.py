...
<head>
    ...
    <title>🏀 Statistiques Basket CSMF</title>
    <style>
        * {
            margin: 0;
            padding: 0;
        ...
        /* Indicateur de résultat - accessible avec texte et icône */
        ...
        .header {
            display: none; /* Caché car on utilise la sidebar */
        }
        ...
        /* ========== COACHING IA ========== */
        .coaching-card { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 30px; border-radius: 15px; margin: 20px 0; box-shadow: 0 10px 30px rgba(0,0,0,0.3); }
        ...
        .lineup-card { background: linear-gradient(135deg, #1e293b 0%, #334155 100%); border-radius: 15px; padding: 20px; margin: 15px 0; color: white; }
        ...
        .advanced-stat-card { background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); padding: 20px; border-radius: 12px; text-align: center; border: 2px solid transparent; transition: all 0.3s; }
        .advanced-stat-card:hover { border-color: #667eea; transform: translateY(-3px); box-shadow: 0 5px 15px rgba(102, 126, 234, 0.2); }
        .advanced-stat-value { font-size: 2em; font-weight: bold; color: #4338ca; }
        ...
        /* Toggle sections */
        .section-toggle { background: #667eea; color: white; border: none; padding: 12px 25px; border-radius: 8px; cursor: pointer; font-size: 1em; margin: 10px 5px; transition: all 0.3s; }
        .section-toggle:hover { background: #5a67d8; transform: translateY(-2px); }
        .section-toggle.active { background: #4c51bf; }
        .collapsible-section { max-height: 0; overflow: hidden; transition: max-height 0.5s ease-out; }
                <small style="color: #ffd700;">coucou</small>
        ...
        .lineup-filter-btn.active { background: #667eea; color: white; }
        ...
        .mobile-header-spacer {
            width: 44px; /* Pour équilibrer le hamburger */
        }
...
<body>
    ...
    <div class="app-layout">
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <div id="uploadModal" class="upload-modal" style="display: none;">
                <div class="upload-modal-content">
                    <div class="upload-modal-header">
                        <h3>📥 Importer un nouveau match</h3>
                        <button class="modal-close" onclick="closeUploadModal()">✕</button>
                    </div>
                    <div class="upload-modal-body">
                        <p class="upload-help">Sélectionnez les 3 fichiers PDF de la fédération :</p>
                        ...
                        <div class="upload-file-row">
                            <label>
                                <span class="file-label">2. Boxscore Détaillée</span>
            ...
            <nav class="sidebar-nav" id="sidebar-nav">
                <!-- Navigation générée par JS -->
            </nav>
            <div class="sidebar-matches" id="sidebar-matches">
                <!-- Liste des matchs générée par JS -->
            </div>
        </aside>
    ...
    <script>
        ...
        let currentView = 'matches'; // 'matches', 'csmf', ou 'autres'
        let currentTab = 'overview';
        let selectedMatchesForCsmf = [];
        let csmfDataCache = null;
        
        // État du tri pour le tableau CSMF
        let csmfSortColumn = 'evaluation'; // Colonne par défaut
        ...
        // Fonction pour obtenir un nom d'équipe court mais lisible
        ...
        async function loadMatches() {
            ...
            if (result.success) {
                ...
                // Trier par journée (J1, J2, J3...)
                allMatches.sort((a, b) => {
                    const jA = a.journee ? parseInt(a.journee.replace('J', '')) : 999;
                    const jB = b.journee ? parseInt(b.journee.replace('J', '')) : 999;
                    return jA - jB;
                });
                selectedMatchesForCsmf = allMatches.map(m => m.id); // Tous sélectionnés par défaut
                renderSidebar();
                if (allMatches.length > 0) {
                    await loadMatch(allMatches[0].id);
                } else {
                    render();
                }
            } else {
        ...
        function renderSidebar() {
            ...
            // Safeguard si les éléments n'existent pas encore
                    ...
                    matchesHtml += `
                        <div class="match-list-item ${isActive ? 'active' : ''}" onclick="loadMatch(${m.id}); closeSidebar();">
                            ...
                            <div class="match-info">
                                ...
                                <div class="match-score-small">${m.score_domicile} - ${m.score_exterieur} • ${m.date || ''}</div>
                            </div>
                            <span class="match-result-indicator ${resultClass}">${resultText}</span>
                            <button class="match-delete-btn" onclick="deleteMatch(${m.id}, event)" title="Supprimer">🗑</button>
                        </div>
                    `;
                });
            }
        ...
        async function deleteMatch(matchId, event) {
            event.stopPropagation(); // Empêcher le clic de sélectionner le match
            ...
            try {
                ...
                if (result.success) {
                    ...
                    await loadMatches();
                    // Si c'était le match actuel, retourner à la vue principale
        ...
        async function uploadMatch() {
            ...
            if (!file.name.toLowerCase().endsWith('.pdf')) {
                messageDiv.className = 'message error show';
                messageDiv.textContent = '❌ Seuls les fichiers PDF sont acceptés';
                return;
            }
            ...
            try {
                ...
                if (result.success) {
                    ...
                    messageDiv.textContent = '✅ Match importé avec succès !';
                ...
                messageDiv.className = 'message error show';
                messageDiv.textContent = '❌ Erreur lors de l\'upload: ' + error.message;
            }
        }
        ...
        async function importMatchFiles() {
            ...
            // Vérifier le fichier obligatoire
            if (!fibaInput.files || fibaInput.files.length === 0) {
                resultDiv.className = 'upload-result error';
                resultDiv.innerHTML = '❌ Le fichier FIBA Box Score est obligatoire';
                return;
            }
            ...
            if (!fibaFile.name.toLowerCase().endsWith('.pdf')) {
                resultDiv.className = 'upload-result error';
                resultDiv.innerHTML = '❌ Seuls les fichiers PDF sont acceptés';
                return;
            }
            ...
            try {
                ...
                if (result.success) {
                    let successMsg = '✅ Match importé avec succès !';
                    if (result.lineups_count > 0) {
                        successMsg += `<br>📊 ${result.lineups_count} combinaisons de 5 ajoutées`;
                    }
                ...
                progressDiv.style.display = 'none';
                resultDiv.className = 'upload-result error';
                resultDiv.innerHTML = '❌ Erreur lors de l\'upload: ' + error.message;
            }
        }
        ...
        async function uploadMatchMultiple() {
            ...
            if (!fibaInput.files || fibaInput.files.length === 0) {
                messageDiv.className = 'message error show';
                messageDiv.textContent = '❌ Le fichier FIBA Box Score est obligatoire';
                return;
            }
            ...
            // Ajouter les fichiers optionnels s'ils sont présents
            if (boxscoreInput.files && boxscoreInput.files.length > 0) {
                formData.append('file_boxscore', boxscoreInput.files[0]);
            }
            if (analyse5Input.files && analyse5Input.files.length > 0) {
            ...
            try {
                ...
                if (result.success) {
                    let successMsg = '✅ Match importé avec succès !';
                    if (result.lineups_count > 0) {
                        successMsg += ` (${result.lineups_count} combinaisons de 5 ajoutées)`;
                    }
        ...
        async function uploadAnalyse5(matchId) {
            ...
            messageDiv.className = 'message show';
            messageDiv.innerHTML = '⏳ Import en cours...';
            ...
            try {
                ...
                if (result.success) {
                    messageDiv.className = 'message success show';
                    messageDiv.textContent = `✅ ${result.count} combinaisons importées !`;
        ...
        // Supprimer les données de combinaisons de 5
        async function deleteLineupsData(matchId) {
            if (!confirm('Êtes-vous sûr de vouloir supprimer les données de combinaisons de 5 ?')) {
                return;
            }
        ...
        // Uploader le fichier Boxscore Détaillée pour un match existant
        async function uploadBoxscore(matchId) {
            ...
            try {
                ...
                if (result.success) {
                    messageDiv.className = 'message success show';
                    messageDiv.textContent = '✅ Stats avancées importées !';
        ...
        // Supprimer les stats avancées
        async function deleteAdvancedStats(matchId) {
            if (!confirm('Êtes-vous sûr de vouloir supprimer les stats avancées ?')) {
                return;
            }
        ...
        // Charger les données CSMF de manière asynchrone
        async function loadCsmfDataAsync() {
            const loadingEl = document.getElementById('csmf-content');
            if (loadingEl) {
                loadingEl.innerHTML = '<div class="loading">⏳ Chargement des statistiques CSMF...</div>';
            }
        ...
        // Onglets principaux de navigation (gardé pour compatibilité mais non utilisé)
            ...
            const matchHeader = currentMatch ? `
                <div class="current-match-header">
                    ...
                    <div class="match-title-info">
                        ...
                        <p>${currentMatch.date || ''} • Score: ${currentMatch.score_domicile} - ${currentMatch.score_exterieur}</p>
                    </div>
                </div>
            ` : '';
        ...
        function renderOverview() {
            ...
            return `
                <div class="tab-content active">
                    ...
                    <h2 style="margin-top: 30px;">🏆 Top 5 Joueuses du Match (par Evaluation)</h2>
                    ${topScorers.map((p, i) => `
                        <div class="top-scorer">
                            ...
                            <div>
                                ...
                                ${p.points || 0} pts • ${p.rebonds_total || 0} reb • ${p.passes_decisives || 0} pd
                            </div>
                        </div>
                    `).join('')}
        ...
        // Section des fichiers optionnels (Boxscore Détaillée et Analyse des 5)
        function renderOptionalFilesSection() {
            ...
            return `
                <div style="margin-top: 30px; padding: 20px; background: #f8fafc; border-radius: 15px; border: 2px solid #e2e8f0;">
                    <h2 style="margin-bottom: 20px;">📁 Données Complémentaires</h2>
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                                <div>
                                    <h3 style="margin: 0; color: ${hasLineups ? '#22c55e' : '#64748b'};">
                                        ${hasLineups ? '✅' : '❌'} Analyse des 5 en jeu
                                    </h3>
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                                <div>
                                    <h3 style="margin: 0; color: ${hasAdvancedStats ? '#22c55e' : '#64748b'};">
                                        ${hasAdvancedStats ? '✅' : '❌'} Boxscore Détaillée
                                    </h3>
                                    <small style="color: #94a3b8;">Stats avancées par période</small>
                                </div>
                            </div>
        ...
        function renderCoachAnalysisForMatch() {
            // Le coaching IA sera inséré automatiquement via renderCoachingForMatch()
        ...
        function renderLineups() {
                ...
                return `
                    <div class="tab-content active">
                        <div class="card" style="text-align: center; padding: 40px;">
                            <h2>🏀 Combinaisons de 5</h2>
                            <p style="color: #666; margin-top: 20px;">
                                Aucune donnée de combinaisons disponible pour ce match.
                            </p>
                            <p style="color: #999; font-size: 0.9em; margin-top: 10px;">
                                Pour importer ces données, utilisez le fichier "Analyse_des_5_en_jeu" de la fédération.
                            </p>
                        </div>
                    </div>
                `;
            ...
            // Déterminer quelle équipe est CSMF
            ...
            return `
                <div class="tab-content active">
                    <h2>🏀 Combinaisons de 5 Joueurs</h2>
                    <p style="color: #666; margin-bottom: 20px;">
                        Analyse de l'efficacité des différentes compositions utilisées pendant le match.
                    </p>
        ...
        // Rendu des combinaisons pour une équipe
        function renderLineupsForTeam(teamName, lineups) {
            // Trier par écart (meilleur en premier)
            const sortedLineups = [...lineups].sort((a, b) => b.ecart - a.ecart);
            
            // Calculer les stats globales
            const totalTime = lineups.reduce((sum, l) => sum + (l.temps_secondes || 0), 0);
            ...
            return `
                <div class="lineup-section">
                    <div class="team-header" style="margin-bottom: 20px;">
                        ${teamName} - ${lineups.length} combinaisons utilisées
                    </div>
                    ...
                    <div class="advanced-stats-grid" style="margin-bottom: 30px;">
                        ...
                        <div class="advanced-stat-card">
                            <div class="advanced-stat-value" style="color: #4ade80;">${bestLineup ? (bestLineup.ecart > 0 ? '+' : '') + bestLineup.ecart : '-'}</div>
                            <div class="advanced-stat-label">Meilleur écart</div>
                        </div>
                        <div class="advanced-stat-card">
                            <div class="advanced-stat-value" style="color: #f87171;">${worstLineup ? (worstLineup.ecart > 0 ? '+' : '') + worstLineup.ecart : '-'}</div>
                            <div class="advanced-stat-label">Pire écart</div>
                        </div>
                        <div class="advanced-stat-card">
                    ...
                    <div class="lineup-filters">
                        ...
                        <button class="lineup-filter-btn" onclick="filterLineups('positive', this)">✅ Positives</button>
                        <button class="lineup-filter-btn" onclick="filterLineups('negative', this)">❌ Négatives</button>
                    </div>
        ...
        function renderLineupCard(lineup) {
            const ecartClass = lineup.ecart > 0 ? 'positive' : (lineup.ecart < 0 ? 'negative' : 'neutral');
            const ecartSign = lineup.ecart > 0 ? '+' : '';
            
            // Parser les joueurs depuis le format "1- NOM P/ 4- NOM C/ ..."
            const playersRaw = lineup.joueurs || '';
            const playersList = playersRaw.split('/').filter(p => p.trim()).map(p => {
                const match = p.trim().match(/(\d+)-\s*([A-Z]+)/i);
                return match ? `#${match[1]} ${match[2]}` : p.trim();
            });
            
            return `
                <div class="lineup-card ${ecartClass}" data-ecart="${lineup.ecart}">
                    <div class="lineup-header">
                        <div class="lineup-ecart ${ecartClass}">${ecartSign}${lineup.ecart}</div>
                        <div class="lineup-meta">
                            <span>⏱️ ${lineup.temps_jeu || '-'}</span>
                            <span>📊 ${lineup.score_pour || 0}-${lineup.score_contre || 0}</span>
                            <span>⚡ ${(lineup.pts_par_minute || 0).toFixed(1)} pts/min</span>
                        </div>
                    </div>
                    ...
                    <div class="lineup-stats">
                        <div class="lineup-stat">
                            <div class="lineup-stat-value">${lineup.rebonds || 0}</div>
                            <div class="lineup-stat-label">Rebonds</div>
                        </div>
                        <div class="lineup-stat">
                            <div class="lineup-stat-value">${lineup.passes_decisives || 0}</div>
                            <div class="lineup-stat-label">Passes</div>
                        </div>
                        <div class="lineup-stat">
                            <div class="lineup-stat-value">${lineup.interceptions || 0}</div>
                            <div class="lineup-stat-label">Interceptions</div>
                        </div>
                        <div class="lineup-stat">
                            <div class="lineup-stat-value">${lineup.balles_perdues || 0}</div>
                            <div class="lineup-stat-label">Balles perdues</div>
                        </div>
                    </div>
                </div>
            `;
        ...
        // Filtrer les combinaisons affichées
        ...
        // Vue joueuses par équipe (2 tableaux)
        ...
        function renderCsmfViewContent() {
            return `
                <div class="card">
                    <h2>🏆 Statistiques Globales CSMF</h2>
                    
                    ${renderMatchFilter()}
                    
                    <div id="csmf-content">
                        ${csmfDataCache ? renderCsmfData(csmfDataCache) : '<div class="loading">⏳ Chargement des statistiques...</div>'}
                    </div>
                </div>
            `;
        }

        // Rendu des données CSMF
        ...
        function renderCsmfBestLineups(lineups) {
            if (!lineups || lineups.length === 0) {
                return `
                    <div class="card" style="margin-top: 30px;">
                        <h2>🏀 Meilleures Combinaisons de 5</h2>
                        <p style="color: #666; text-align: center; padding: 20px;">
                            Aucune donnée de combinaisons disponible.<br>
            ...
            // Fonction pour normaliser les joueurs d'une combinaison (ordre alphabétique des noms)
            function normalizeLineupKey(joueurs) {
                return joueurs.split('/').map(p => p.trim()).filter(p => p).sort().join('/');
            }
            
            // Agréger les combinaisons identiques sur tous les matchs
            ...
            lineups.forEach(l => {
                ...
                lineupAggregates[key].total_temps_secondes += l.temps_secondes || 0;
                lineupAggregates[key].total_score_pour += l.score_pour || 0;
                lineupAggregates[key].total_score_contre += l.score_contre || 0;
                lineupAggregates[key].total_ecart += l.ecart || 0;
            ...
            }).filter(l => l.temps_total >= 120); // Minimum 2 minutes de jeu pour être significatif
            
            // Trier par écart moyen (moyenne sur tous les matchs)
            ...
            // Fonction pour formatter une carte de combinaison agrégée
            function renderAggregatedLineupCard(lineup, isPositive) {
                ...
                const players = lineup.joueurs.split('/').map(p => {
                    const match = p.trim().match(/(\d+)-\s*([A-Z]+)/i);
                ...
                return `
                    <div style="background: ${bgColor}; border-left: 4px solid ${color}; padding: 15px; margin-bottom: 12px; border-radius: 0 10px 10px 0;">
                        ...
                        <div style="color: #64748b; font-size: 11px; margin-top: 8px;">
                            Écart total: ${lineup.ecart_total >= 0 ? '+' : ''}${lineup.ecart_total} (${lineup.pts_pour}-${lineup.pts_contre})
                        </div>
                    </div>
                `;
            }
            ...
            return `
                <div class="card" style="margin-top: 30px;">
                    <h2>🏀 Analyse des Combinaisons de 5</h2>
                    <p style="color: #666; margin-bottom: 20px; font-size: 14px;">
                        📊 Moyennes calculées sur l'ensemble des matchs (min. 2 min de jeu par combinaison)
                    </p>
                    ...
                    <div class="advanced-stats-grid">
                        ...
                        <div class="advanced-stat-card">
                            ...
                            <div class="advanced-stat-label">Moy. négative</div>
                        </div>
                        <div class="advanced-stat-card">
                            <div class="advanced-stat-value" style="color: ${avgEcart >= 0 ? '#16a34a' : '#dc2626'};">${avgEcart >= 0 ? '+' : ''}${avgEcart.toFixed(1)}</div>
                            <div class="advanced-stat-label">Écart moyen global</div>
                        </div>
                    </div>
                    
                    <!-- Meilleures combinaisons -->
                    ${bestLineups.length > 0 ? `
                        <h3 style="margin-top: 30px; color: #16a34a;">✅ Top 5 Meilleures Combinaisons (moyenne)</h3>
                    ...
                    ${worstLineups.length > 0 ? `
                        <h3 style="margin-top: 30px; color: #dc2626;">⚠️ Combinaisons à éviter (moyenne)</h3>
                        <div style="margin-top: 15px;">
        ...
        function renderCoachAnalysisGlobal(csmfData) {
            ...
            // Le coaching IA global sera inséré automatiquement via renderGlobalCoaching()
                        ...
                        <div class="checkbox-group">
                            ...
                            ${allMatches.map(m => `
                                <label class="checkbox-label">
                                    ...
                                    ${m.journee ? `[${m.journee}] ` : ''}${getShortTeamName(m.equipe_domicile)} vs ${getShortTeamName(m.equipe_exterieur)} (${m.date})
                                </label>
                            `).join('')}
                        </div>
                    </div>
        ...
        async function loadCsmfData() {
            ...
            for (const matchId of selectedMatchesForCsmf) {
                ...
                if (result.success) {
                    ...
                    // Pour éviter les doublons dans un même match
                    ...
                    players.forEach(p => {
                        ...
                        if (p.equipe && p.equipe.includes('CSMF')) {
                            console.log(`  CSMF trouvée: ${p.nom} - ${p.points} pts - Equipe: ${p.equipe}`);
                        }
                        ...
                        if (p.equipe && p.equipe.includes('CSMF') && isRealPlayer) {
                            // Pour CSMF : utiliser le nom de famille comme clé d'agrégation
                            const lastName = getLastName(p.nom);
                            
                            // Vérifier si cette joueuse (par nom de famille) a déjà été traitée dans ce match
                            if (processedInThisMatch.has(lastName)) {
                                console.log(`  ⚠️ DOUBLON DETECTE (même nom de famille): ${p.nom}`);
                            ...
                            // Utiliser le nom de famille comme clé pour l'agrégation
                            if (!csmfPlayers[lastName]) {
                                csmfPlayers[lastName] = {
                                    nom: p.nom, // Garder le premier prénom complet rencontré
                                    matchs: 0,
                                    points: 0,
                                    rebonds: 0,
                                    rebonds_off: 0,
                                ...
                                console.log(`  ✨ Première occurrence: ${p.nom} → clé: ${lastName}`);
                            } else {
                                console.log(`  🔄 Fusion avec ${csmfPlayers[lastName].nom}: ${p.nom} - ${p.points} pts`);
                            }
                            
                            console.log(`  ✓ Ajout: ${p.nom} - ${p.points} pts`);
                    ...
                    console.log(`Joueuses CSMF traitées: ${csmfCount}`);
                    console.log(`Total points CSMF ce match: ${totalPointsThisMatch}`);
                }
            }
            
            // Charger les combinaisons de 5 pour tous les matchs sélectionnés
            let allLineups = [];
            for (const matchId of selectedMatchesForCsmf) {
                try {
                    const lineupsResponse = await fetch(`${API_URL}/matches/${matchId}/lineups`);
        ...
        function renderCsmfSummary(csmfData, teamStats) {
            ...
            // Calculer les pourcentages de réussite globaux
            ...
            return `
                <div class="summary-stats">
                    <h3>📊 Résumé sur ${nbMatchs} match(s)</h3>
                    <div class="summary-grid">
                        ...
                        <div class="summary-item">
                            <div class="label">Moy Rebonds Défensifs</div>
                        ...
                        <div class="summary-item">
                            <div class="label">% Réussite LF Adversaires</div>
        ...
        function renderCsmfPlayersTable(csmfData) {
            ...
            // Helper pour générer la classe de tri
            const sortClass = (col) => {
            ...
            return `
                <h2 style="margin-top: 30px;">Statistiques Détaillées des Joueuses CSMF</h2>
                <p style="color: #666; font-size: 0.9em; margin-bottom: 15px;">💡 Cliquez sur les en-têtes de colonnes pour trier</p>
                <div class="scroll-table">
        ...
        function sortCsmfTable(column) {
            ...
            if (csmfSortColumn === column) {
                ...
                // Par défaut : desc pour les stats numériques, asc pour le nom
                ...
                <div class="tab-content active">
                    ...
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            ...
                            <small>marqués au total</small>
                        </div>
                        <div class="stat-card">
                            <h3>Écart</h3>
                            <div class="value">${ecart}</div>
                            <small>points de différence</small>
                        </div>
                    </div>
        ...
        // Upload d'un match non-CSMF avec contrôle
        async function uploadNonCsmfMatch() {
            ...
            messageDiv.className = 'message show';
            messageDiv.innerHTML = '<span class="loading"></span> Vérification et import...';
            ...
            try {
                ...
                if (result.success) {
                    // Vérifier si le match contient CSMF
                    ...
                    if (match.equipe_domicile.includes('CSMF') || match.equipe_exterieur.includes('CSMF')) {
                        ...
                        messageDiv.className = 'message error show';
                        messageDiv.textContent = '❌ Ce match contient le CSMF ! Utilisez l\'onglet "Analyse par Match"';
                        fileInput.value = '';
                        return;
                    }
        ...
        async function deleteNonCsmfMatch(matchId, event) {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    // Réinitialiser si c'était l'équipe sélectionnée
                    if (selectedTeam) {
        ...
        async function loadTeamData() {
            ...
            // Filtrer les matchs selon les adversaires sélectionnés
            ...
            for (const match of filteredMatches) {
                ...
                if (result.success) {
                    ...
                    // Filtrer les joueuses de l'équipe sélectionnée
                    players.forEach(p => {
        ...
        // Vue principale Autres Équipes
        function renderAutresView() {
            const teams = getUniqueTeams();
            
            return `
                <div class="card">
                    <h2>🔍 Analyse des Autres Équipes</h2>
                    
                    <!-- Section d'upload -->
                    <div class="upload-section">
                        <h3>➕ Ajouter un match (sans CSMF)</h3>
                        ...
                        <p style="color: #999; font-size: 13px; margin-top: 10px;">
                            ⚠️ Les matchs contenant le CSMF seront automatiquement rejetés
                        </p>
                    </div>

                    <!-- Liste des matchs non-CSMF -->
                    <h3 style="margin-top: 30px;">📋 Matchs disponibles (${allNonCsmfMatches.length})</h3>
                            ...
                            ${allNonCsmfMatches.map(m => `
                                <div style="position: relative;">
                                    <button class="match-btn" style="width: 100%; text-align: left; cursor: default;">
                                        ...
                                        <br><small>${m.date} • ${m.score_domicile}-${m.score_exterieur}</small>
                                    </button>
                    ...
                    ` : '<p style="color: #999;">Aucun match importé</p>'}
                    
                    ${teams.length > 0 ? `
                        <!-- Sélection d'équipe -->
                        <h3 style="margin-top: 30px;">🎯 Sélectionner une équipe à analyser</h3>
        ...
        // Analyse de l'équipe sélectionnée
        function renderTeamAnalysis() {
            ...
            return `
                <div style="margin-top: 30px; padding: 20px; background: #f8f9ff; border-radius: 10px;">
                    ...
                    ${selectedOpponents.length > 0 && teamStatsCache ? renderTeamStats() : '<p style="color: #999;">Sélectionnez au moins un adversaire</p>'}
                </div>
            `;
        }
                ...
                <div style="margin-top: 20px;">
                    <div class="summary-stats">
                        ...
                        <div class="summary-grid">
                            ...
                            <div class="summary-item">
                                <div class="label">Points marqués</div>
                                <div class="value">${totalPoints}</div>
                            </div>
                            <div class="summary-item">
                    ...
                    <h3 style="margin-top: 30px;">👥 Statistiques des Joueuses</h3>
                    <div class="scroll-table">
                        <table>
                            <thead>
                                <tr>
        ...
        async function renderCoachingForMatch(match) {
            ...
            if (!c) return;
            c.innerHTML = '<div class="coaching-card"><h2>🤖 Analyse IA</h2><div class="coaching-loader"><div class="spinner"></div><p>Analyse en cours... ⏳</p></div></div>';
            const ok = await checkOllamaStatus();
            if (!ok) { 
                c.innerHTML = '<div class="coaching-card coaching-error"><h2>⚠️ IA Non Disponible</h2><p>Lancez: <code>python api_server_ollama.py</code></p></div>'; 
                return; 
            }
            const a = await analyzeMatchWithAI(match.id);
            if (!a || !a.success) { 
                c.innerHTML = '<div class="coaching-card coaching-error"><h2>❌ Erreur</h2></div>'; 
                return; 
            }
            ...
            const vic = s.victoire ? '✅ VICTOIRE' : '❌ DÉFAITE';
            c.innerHTML = '<div class="coaching-card"><h2>🤖 Analyse IA</h2><div class="coaching-summary"><strong>' + vic + '</strong> contre ' + s.adversaire + ' (' + s.score_csmf + '-' + s.score_adv + ')</div><div class="coaching-section"><h3>💪 Points Forts</h3><ul class="coaching-list">' + f.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-section"><h3>🎯 À Améliorer</h3><ul class="coaching-list coaching-warning">' + w.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-section"><h3>📋 Conseils</h3><ul class="coaching-list coaching-advice">' + co.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-footer"><small>💡 IA Llama 3.2</small><button onclick="refreshCoaching(' + match.id + ')" class="btn-refresh">🔄</button></div></div>';
        }
        
        async function renderGlobalCoaching(matchIds, playersStats) {
            const c = document.getElementById('global-coaching-section');
            if (!c) return;
            c.innerHTML = '<div class="coaching-card"><h2>🤖 Bilan</h2><div class="coaching-loader"><div class="spinner"></div><p>Analyse...</p></div></div>';
            const ok = await checkOllamaStatus();
            if (!ok) { 
                c.innerHTML = '<div class="coaching-card coaching-error"><h2>⚠️ IA Indisponible</h2></div>'; 
                return; 
            }
            ...
            const s = a.summary;
            c.innerHTML = '<div class="coaching-card"><h2>🤖 Bilan CSMF</h2><div class="coaching-summary">📊 ' + s.nb_matchs + ' matchs • ' + s.victoires + 'V-' + s.defaites + 'D • ' + s.avg_points + 'pts/match</div><div class="coaching-section"><h3>📊 Tendances</h3><ul class="coaching-list">' + t.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-section"><h3>💪 Forces</h3><ul class="coaching-list">' + f.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-section"><h3>🎯 Progression</h3><ul class="coaching-list coaching-warning">' + ax.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-section"><h3>🔄 Stratégie</h3><ul class="coaching-list coaching-advice">' + r.map(function(x){ return '<li>' + x + '</li>'; }).join('') + '</ul></div><div class="coaching-footer"><small>💡 IA Llama 3.2</small></div></div>';
        }
        ...
        // ========================================
        // PAGE DÉDIÉE JOUEUSE (Style NBA)
        // ========================================
        
        async function showPlayerDetails(playerName) {
            console.log(`Affichage des détails pour: ${playerName}`);
            ...
            if (!playerData || playerData.length === 0) {
                alert(`Aucune donnée trouvée pour ${playerName}`);
                return;
            }
            ...
            for (const matchId of selectedMatchesForCsmf) {
                ...
                if (result.success) {
                    ...
                    try {
                        ...
                        const lineupsResult = await lineupsResponse.json();
                        if (lineupsResult.success && lineupsResult.data) {
                            // Filtrer les combinaisons CSMF où la joueuse apparaît
            ...
            modal.innerHTML = `
                <div style="max-width: 1200px; margin: 0 auto; background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); border-radius: 20px; padding: 40px; position: relative;">
                    ...
                    <div style="text-align: center; margin-bottom: 40px;">
                        ...
                        <p style="color: #aaa; font-size: 20px;">CSMF PARIS • #${playerData[0].numero || '?'}</p>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 40px;">
                        <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px;">
                            <h3 style="color: white; margin-bottom: 20px;">📈 Évolution Points</h3>
                            <canvas id="chart-points" style="max-height: 250px;"></canvas>
                        </div>
                        <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px;">
                            <h3 style="color: white; margin-bottom: 20px;">📊 Évolution EVAL</h3>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-bottom: 40px;">
                        <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px;">
                            <h3 style="color: white; margin-bottom: 20px;">🎯 Répartition Paniers</h3>
                            <canvas id="chart-scoring" style="max-height: 250px;"></canvas>
                        </div>
                        <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px;">
                            <h3 style="color: white; margin-bottom: 20px;">⭐ Profil Joueur</h3>
                    ...
                    <!-- Stats par match -->
                    <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px;">
                        <h3 style="color: white; margin-bottom: 20px;">📋 Détails par Match</h3>
        ...
        function renderPlayerLineupsSection(playerName, lineups) {
            if (!lineups || lineups.length === 0) {
                return `
                    <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px; margin-bottom: 40px;">
                        <h3 style="color: white; margin-bottom: 15px;">🏀 Combinaisons de 5</h3>
                        <p style="color: #aaa; text-align: center;">Aucune donnée de combinaisons disponible</p>
                    </div>
                `;
            }
            ...
            // Fonction pour normaliser les joueurs d'une combinaison (ordre alphabétique)
            function normalizeLineupKey(joueurs) {
                return joueurs.split('/').map(p => p.trim()).filter(p => p).sort().join('/');
            }
            ...
            // Calculer les stats globales sur les données agrégées
            ...
            // Fonction pour formater les coéquipières (sans la joueuse elle-même)
            ...
            return `
                <div style="background: rgba(255,255,255,0.05); border-radius: 15px; padding: 25px; margin-bottom: 40px;">
                    <h3 style="color: white; margin-bottom: 20px;">🏀 Combinaisons de 5 avec ${playerName.split(' ').pop()}</h3>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        <!-- Meilleures combinaisons -->
                        <div>
                            <h4 style="color: #4ade80; margin-bottom: 15px;">✅ Meilleures associations (moyenne)</h4>
                            ${bestLineups.length > 0 ? bestLineups.map(l => `
                                <div style="background: rgba(74, 222, 128, 0.1); border-left: 4px solid #4ade80; padding: 12px; margin-bottom: 10px; border-radius: 0 8px 8px 0;">
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        ...
                                        <span style="color: #aaa; font-size: 12px;">${l.nb_matchs} match${l.nb_matchs > 1 ? 's' : ''} • ${l.temps_display}</span>
                                    </div>
                                    <div style="color: white; margin-top: 8px; font-size: 14px;">
                                        avec ${formatTeammates(l.joueurs)}
                                    </div>
                                    <div style="color: #666; font-size: 11px; margin-top: 4px;">
                                        Écart total: +${l.ecart_total} • ${l.pts_par_min} pts/min
                                    </div>
                                </div>
                        ...
                        <!-- Pires combinaisons -->
                        <div>
                            <h4 style="color: #f87171; margin-bottom: 15px;">⚠️ Associations difficiles (moyenne)</h4>
                            ...
                            `).join('') : '<p style="color: #aaa;">Aucune combinaison négative</p>'}
                        </div>
                    </div>
                </div>
            `;
        ...
        function drawPlayerCharts(playerName, playerData) {
            ...
            console.log('Création des graphiques pour:', playerName);
            console.log('Données:', playerData);
            
            // 1. Graphique Évolution Points
            const ctxPoints = document.getElementById('chart-points');
            if (ctxPoints) {
                console.log('Canvas points trouvé, création du graphique...');
                new Chart(ctxPoints, {
                    type: 'line',
                    data: {
                ...
                console.error('Canvas chart-points non trouvé !');
            }
            
            // 2. Graphique Évolution EVAL
            ...
            // 3. Graphique Répartition Paniers (Doughnut) - Nombre de paniers réussis
            ...
            if (ctxScoring) {
                ...
                new Chart(ctxScoring, {
                    type: 'doughnut',
                    data: {
                        labels: ['2pts réussis', '3pts réussis', 'LF réussis'],
                        datasets: [{
